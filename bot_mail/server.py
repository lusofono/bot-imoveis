...
def create_server(folder, public_url=None):
    ...
    async def run(function, *args):
        # IMAP/SMTP block for seconds; a worker thread keeps OAuth and other requests responsive.
...
def http_app(folder, public_url):
    ...
    # RFC 9728 clients follow the /mcp-suffixed URL from the 401 header; ChatGPT's guide also names the root path.
    protected = next(route for route in app.router.routes
                     if getattr(route, "path", "") == "/.well-known/oauth-protected-resource/mcp")
    app.router.routes.append(Route("/.well-known/oauth-protected-resource", protected.endpoint, methods=protected.methods))
...
class TokenResourceMiddleware:
    """A token request may omit resource (refreshes often do); it must never name another one.
    Tokens are only ever issued for this server's own resource."""
