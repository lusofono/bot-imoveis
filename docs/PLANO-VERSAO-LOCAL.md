...
2. **Visitas fechadas (feito a 22/09):** o texto fica na voz. Um botão por imóvel («Imóveis» → Visitas)
   ...
   depois do envio: ver `docs/DECISOES.md`, 22/09), os pendentes ficam com o texto de fecho em vez do
   normal, e os pedidos novos desse anúncio chegam já com o texto preparado (`reply_status: draft`).
3. **Lembretes aos 2 e aos 4 dias sem resposta (feito a 22/09):** duas frases na voz. Em cada leitura,
   preparam-se automaticamente na mesma conversa (`Re:`, `In-Reply-To` do nosso último envio real): a frase
   por cima do último texto que enviámos, que passa a ficar guardado na conversa. Param se o cliente
   responder, se as visitas fecharem ou se o rascunho do lembrete for retirado da fila; no máximo dois.
   «Sem resposta» conta sempre pelo silêncio do cliente, nunca por saber se abriu o email (ver
   `docs/DECISOES.md`, 22/09: mantém-se a decisão de 21/09 de não usar píxeis nem recibos de leitura).
4. **Pedido de consentimento (feito a 22/09):** o texto fica na voz, e há um botão por imóvel para todos os
